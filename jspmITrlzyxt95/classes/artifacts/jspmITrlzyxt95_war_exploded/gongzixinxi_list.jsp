...
<form name="form1" id="form1" method="post" action="">
    ...
    <input type="submit" name="Submit" value="查找" style='border:solid 1px #000000; color:#666666'/> <input type="button"
                                                                                                           ...
                                                                                                           onClick="javascript:location.href='gongzixinxi_listxls.jsp';"/>
</form>
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF"
    ...
    <tr>
        ...
        <td><%=map.get("jiabanfei") %>
        </td>
        <td><%=map.get("jiangjin") %>
        </td>
        <td><%=map.get("chengjin") %>
        </td>
        <td><%=map.get("kouchuqingjiafeiyong") %>
        </td>
        <td><%=map.get("kouchukuanggongfeiyong") %>
        </td>
        <td><%=map.get("kouchushuifei") %>
        </td>
        <td><%=map.get("kouchubaoxianfei") %>
        </td>
        <td><%=map.get("kouchugongjijin") %>
        </td>
        <td><%=map.get("shifagongzi") %>
        </td>


        <td width="138" align="center"><%=map.get("addtime") %>
        </td>
        <td width="60" align="center"><a href="gongzixinxi_updt.jsp?id=<%=map.get("id")%>">修改</a> <a
                href="gongzixinxi_list.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a> <a
                href="gongzixinxi_detail.jsp?id=<%=map.get("id")%>">详细</a></td>
    </tr>
    <%
        }
    %>
